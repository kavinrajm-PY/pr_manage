...
// src/app/lead/projects/page.tsx
// Team Lead page to list all projects they are assigned to.
  ...
  useEffect(() => {
    async function loadProjects() {
      ...
      try {
        ...
        console.error('Failed to load lead projects', error);
      } finally {
        setLoading(false);
      }
    }
    ...
    return (
      <AppLayout>
        <div className="flex flex-col items-center justify-center h-[50vh] text-center space-y-2">
          ...
          <p className="text-muted-foreground">Only Team Leads can access this page.</p>
        </div>
      </AppLayout>
    );
  }
      ...
      <div className="space-y-6">
        ...
        {projects.length === 0 ? (
          <div className="rounded-lg border border-dashed p-8 text-center bg-card text-muted-foreground">
            ...
            <p className="text-sm">You are not currently assigned as a Lead to any project.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map((project) => (
              <ProjectCard key={project.id} project={project} hrefPrefix="/lead/projects" />
            ))}
          </div>
        )}
      </div>
